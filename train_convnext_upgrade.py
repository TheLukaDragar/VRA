...
class ConvNeXt(pl.LightningModule):
    def __init__(self,og_path, model_name='convnext_tiny', dropout=0.1):
        ...
        self.iqa_loss = IQALoss(loss_type='norm-in-norm', alpha=[1, 0], beta=[.1, .1, 1], p=1, q=2, monotonicity_regularization=True, gamma=0.1, detach=False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        iqaloss = self.norm_loss_with_normalization(preds, y, p=1, q=2)
        kldivloss = self.kl_divergence_loss(preds, y)
        loss = iqaloss + kldivloss
        rmseloss = self.RMSE(preds, y)
        print(f"shape of preds {preds.shape} shape of y {y.shape}")
        print(f"loss {loss} iqaloss {iqaloss} kldivloss {kldivloss} rmseloss {rmseloss}")
        self.log('train_loss', loss.item(), on_epoch=True, prog_bar=True,sync_dist=True)
        self.log('train_iqaloss', iqaloss.item(), on_epoch=True, prog_bar=True,sync_dist=True)
        self.log('train_kldivloss', kldivloss.item(), on_epoch=True, prog_bar=True,sync_dist=True)
        self.log('train_rmseloss', rmseloss.item(), on_epoch=True, prog_bar=True,sync_dist=True)
...
if __name__ == '__main__':
    ...
    face_frames_dataset =RandomSeqFaceFramesDataset(dataset_root, labels_file,transform=transform_train,seq_len=seq_len,seed=42)
    
    print("splitting dataset")
    train_ds, val_ds, test_ds = train_val_test_split(face_frames_dataset,seed=42)
