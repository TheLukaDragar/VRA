...
from dataset_tool import (
    MulRandomSeqFaceFramesDataset,
    FaceFramesSeqPredictionDatasetFinal,
)
...
def train_val_split(dataset, num_slices, train_prop=0.8, val_prop=0.2, seed=None):
    total_videos = len(dataset) // num_slices
    video_indices = list(range(total_videos))
    assert (
        len(dataset) % num_slices == 0
    ), "Dataset length must be divisible by num_slices"

    if seed is not None:
        random.seed(seed)

    random.shuffle(video_indices)
    split_idx = int(train_prop * total_videos)

    train_video_indices = video_indices[:split_idx]
    val_video_indices = video_indices[split_idx:]
    ...
    for i in range(num_slices):
        train_indices.extend(
            [index + i * total_videos for index in train_video_indices]
        )
        val_indices.extend([index + i * total_videos for index in val_video_indices])
...
class Eva(pl.LightningModule):
    def __init__(
        ...
        elif loss == "norm_loss_with_normalization":
            self.loss_fn = self.norm_loss_with_normalization
    ...
    def pairwise_ranking_loss(self, preds, y):
        return torch.mean(torch.clamp(preds.view(-1) - y.view(-1), min=0))
    ...
    def on_test_epoch_end(self):
        ...
        # log loss fnc and loss
        self.log(
            self.current_test_set + "_loss",
            self.loss_fn(test_preds, test_labels),
            sync_dist=True,
        )
...
if __name__ == "__main__":
    ...
    face_frames_dataset = MulRandomSeqFaceFramesDataset(
        dataset_root,
        labels_file,
    ...
    train_ds, val_ds = train_val_split(face_frames_dataset, args.num_slices)
